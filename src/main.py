...
print("mode:", mode, "evaluation:", evaluation, 'visualization:', visualization)
...
dataloader_val = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=batch_size, num_workers=10, collate_fn=collate, shuffle=True, pin_memory=True)
...
evaluator = Evaluator(n_class, size_g, size_p, sub_batch_size, mode, visualization, task_name)
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            ...
            log = log + "val sn:" + str(score_val["sensitivity"]) + "\n"
            log = log + "val sp:" + str(score_val["specificity"]) + "\n"
            log = log + "val F1:" + str(score_val["fmeasure"]) + "\n"
            log = log + "Local train:" + str(score_train_local["accuracy"]) + "\n"
            log = log + "Local val acc:" + str(score_val_local["accuracy"]) + "\n"
            log = log + "Global train:" + str(score_train_global["accuracy"]) + "\n"
            log = log + "Global val acc:" + str(score_val_global["accuracy"]) + "\n"
            log += "================================\n"
            print(log)
