...
def CAM_on_image(ori_image, activation, name):
    # colormap
    cm = matplotlib.cm.get_cmap('jet')
    # add colormap to activation
    heatmap = cm(activation)
    # add transp
    trans_map = copy.copy(heatmap)
    trans_map[:,:,3] = 0.4
    trans_map = Image.fromarray((trans_map*255).astype(np.uint8))
    heatmap = Image.fromarray((heatmap*255).astype(np.uint8))
    # overlay
    coi = Image.new('RGBA', ori_image.size)
    coi = Image.alpha_composite(coi, ori_image.convert('RGBA'))
    coi = Image.alpha_composite(coi, trans_map)
    coi.save('%s.png'%name)
                    ...
                    if self.mode == 2:
                        ...
                        for i in range(len(images)):
                            ...
                            while j < len(coordinates[i]):
                                ...
                                #model.module.fpn_local.smooth.register_forward_hook(get_activation('smooth')) # get patch level activation
