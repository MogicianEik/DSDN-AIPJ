...
def majority_voting():
    gt_raw = open("data/KPMP_raw.txt", "r")
    gt = open("data/KPMP_gt.txt", "w")
    lines_raw = gt_raw.readlines()

    for line_raw in lines_raw:
        voting = {'HEALTHY':0, 'DKD':0, 'none':-1}
        ...
        if sorted_voting[0][1] == sorted_voting[1][1]:
            if sorted_voting[0][0] == 'H-CKD' or sorted_voting[1][0] == 'H-CKD':
                label = 'H-CKD'
            else:
                label = sorted_voting[0][0]
        '''        
...
def split_files():
    gt = open("data/KPMP_gt.txt", "r")
    gt_train= open("data/gt_train.txt", "w+")
    gt_test = open("data/gt_test.txt", "w+")
    lines = gt.readlines()
    GT_all = {}
    for line in lines:
    ...
    GT_class = {'HEALTHY':[], 'DKD':[]}
    for label in GT_all.keys():
        GT_class[GT_all[label]].append(label)
    print('HEALTHY ' + str(len(GT_class['HEALTHY'])))
    split_train_test(GT_class['HEALTHY'],gt_train, gt_test, GT_all)
    
    split_train_test(GT_class['DKD'],gt_train, gt_test, GT_all)
    return
...
def get_gt(path, path_down='data/globals'):
    ...
    for line in lines:
        ...
        GT[name] = txt2num.index(label.upper())
    return GT, valid_ids
